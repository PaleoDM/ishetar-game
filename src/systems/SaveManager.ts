...
// Default hero stats at level 1
const DEFAULT_HERO_STATS: Record<string, { hp: number; mana: number | null; ki?: number }> = {
  vicas: { hp: 10, mana: null, ki: 5 },
  azrael: { hp: 9, mana: null },
  lyra: { hp: 10, mana: 10 },
  thump: { hp: 10, mana: 10 },
  rooker: { hp: 10, mana: 10 },
};
...
export class SaveManager {
  ...
  static createHeroStateAtLevel(level: number): Record<string, HeroState> {
    ...
    for (const heroId of Object.keys(DEFAULT_HERO_STATS)) {
      state[heroId] = {
        ...
        ...(heroId === 'vicas' && { currentKi: this.getMaxKi(level) }),
        equipment: null,
        permanentBonuses: createDefaultPermanentBonuses(),
      };
    }
  ...
  static getMaxHp(heroId: string, level: number): number {
    // HP growth tables from Phase 5
    const hpTables: Record<string, number[]> = {
      vicas: [10, 18, 27, 36, 44],
      azrael: [9, 13, 17, 21, 25],
      lyra: [10, 15, 20, 25, 30],
      thump: [10, 14, 19, 24, 28],
      rooker: [10, 12, 15, 17, 20],
    };
  ...
  static getMaxMana(heroId: string, level: number): number | null {
    if (heroId === 'azrael') return null; // No mana
    if (heroId === 'vicas') return null; // Uses Ki instead
  ...
  /**
   * Get max Ki for Vicas at their current level
   */
  ...
  static restoreAllResources(heroState: Record<string, HeroState>): Record<string, HeroState> {
    ...
    for (const [heroId, state] of Object.entries(heroState)) {
      restored[heroId] = {
        ...
        ...(heroId === 'vicas' && { currentKi: this.getMaxKi(state.level) }),
