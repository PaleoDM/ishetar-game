...
export function resolveAttack(
  ...
  const attackRoll = rollAttack(attacker.attack);

  // Get defender's effective defense (includes buffs/debuffs)
...
export function resolveSpell(
  ...
  // Save roll: d20 + resilience vs caster's magic
  const saveRoll = rollSave(target.resilience);
...
export function formatSpellResult(result: SpellResult): string {
  ...
  if (ability.targetType === 'enemy' && saveRoll.dice !== 'none') {
    text += `Save: ${saveRoll.rolls[0]} + ${target.resilience} = ${saveRoll.finalTotal} vs MAG ${targetNumber}\n`;
