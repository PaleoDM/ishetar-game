...
export class BattleScene extends Phaser.Scene {
  ...
  private battleMap: string = 'south_gate';
  ...
  create(data: BattleSceneData): void {
    ...
    this.battleMap = data.battleMap || 'south_gate';
    ...
    if (this.battleConfig.introCutscene && this.battleConfig.introCutscene.length > 0) {
  ...
  private setupMap(): void {
    ...
    this.mapImage = this.add.image(0, 0, `map_${this.battleMap}`);
  ...
  private placeHeroes(): void {
    // Get all 5 heroes in order
    const heroIds = ['vicas', 'azrael', 'lyra', 'thump', 'rooker'];
    ...
    heroIds.forEach((heroId, index) => {
      ...
      if (savedState) {
        ...
        if (savedState.currentMana !== null && savedState.currentMana !== undefined) {
          if (heroId === 'vicas') {
            unit.currentKi = Math.min(savedState.currentMana, unit.maxKi || 0);
          } else {
  ...
  private placeEnemies(): void {
    ...
    this.battleConfig.enemies.forEach((placement) => {
      ...
      // Set enemy facing direction (configurable per battle, default north)
      const enemyFacing = this.battleConfig.enemyFacing || 'north';
  ...
  private syncHeroStateFromUnits(): void {
    for (const hero of this.heroUnits) {
      ...
      // Sync mana/ki based on hero type
      if (heroId === 'vicas') {
        state.currentKi = Math.max(0, Math.min(hero.currentKi || 0, hero.maxKi || 0));
      } else if (hero.maxMana !== undefined && hero.maxMana > 0) {
